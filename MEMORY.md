...
**Nome:** Tabuada Rush  
**Versão:** 6.0.43 (Reset completo + Poções de XP (Fase 4) + Loja rotativa (Fase 5) + Baús/recompensas (Fase 6) + Resumo pós-partida (Fase 7) + revisão visual bloco 1 (Fase 7.1 bloco 1) + catálogo de ícones `ICONES.md` (Fase 7.2) + arte nova/fundos por recurso + screenshots + fundo em tela cheia + FASE 7.1 concluída + Arena no desktop · sessao-094; backlog grande em andamento, ver `PLANO_ACAO.md`)  
